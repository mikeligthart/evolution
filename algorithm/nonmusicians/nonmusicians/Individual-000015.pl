...
sub add_to_loop {
  my ($spc,$t,$s,$seen) = @_;
  my @n = (4, 1, (2+1), 5, 7, 3, 3, 3, (2+1), 5, ((6+1)+(3+2))*5, (5+8));
  @n = map $_ % $nn, @n;
  if (!$seen->{rdg}++) {
  my $l = zeroes($t);

  if ('tune') {
    my $sound = q[wah(asynth(0.25, $n[$note % @n], $len, $spc,  mseries([3, 0.7311, 0.5938, 0, 0.5398, 2, 7, 1, -(3/4), 1/2], 2, 7, 0.8352), [6, 0.03598, 0.9008, 7, 0.6808, 1, 5, 4*2, 2*2*3, 2],  mseries([2, 0.1044, 0.3656, 7, 0.4218, 2, ((2+7)+2)*3, 7, -(1/1), 1/4], 4, 1, 0.9352), [1, 0.3476, 0.4994, 0, 0.03956, 1, (1+1), 3, 2, 1]),0.8367*(5),0.6388*0.02978,0.75)];
...
$l = offset($l,2*4*4+1*4+3);
  ...
  if ('sequencer') {
    my $sound = q[asynth(2, $n[$note % @n], $len, $spc,  mseries([4, 0.5167, 0.5926, 3, 0.7448, 6*4*2, 6, 4, -(2/3), 1/2], 1, 6, 0.2817), [3, 0.7784, 0.9091, (3+6), 0.8357, 4, 5, 4*2, 6*4*2, 2],  mseries([6, 0.5511, 0.1048, 4, 0.4676, 4, 4, 1, -(3/3), 1/4], 1, 9, 0.8542))];
